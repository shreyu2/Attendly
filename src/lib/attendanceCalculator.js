 ...
 */
export function calculateAttendancePercentage(attended, conducted) {
    if (conducted <= 0)
        return null;
 ...
 */
export function calculateSafeSkips(attended, conducted, targetPercentage) {
    if (conducted <= 0 || targetPercentage <= 0 || targetPercentage >= 100)
        return 0;
 ...
 */
export function calculateRecoveryNeeded(attended, conducted, targetPercentage) {
    if (conducted <= 0 || targetPercentage <= 0)
        return 0;
    if (targetPercentage >= 100)
        return 0; // If 100% target and missed any, mathematically impossible
    const currentRatio = (attended / conducted) * 100;
 ...
 */
export function computeSubjectStats(subject, records, globalTarget) {
 ...
 */
export function computeOverallStats(subjects, records, globalTarget) {
    let totalAttended = 0;
    let totalAbsent = 0;
 ...
 */
export function simulateAttendance(currentAttended, currentConducted, targetPercentage, futureAttended, futureSkipped) {
 ...
 */
export function evaluateCanSkipTomorrow(tomorrowDate, subjects, timetable, records, globalTarget) {
